    ...
    async def deposit(
        ...
        try:
            ...
            if 'error' in wallet:
                ...
                await ctx.reply(f"{EMOJI_RED_NO} {error_msg}", ephemeral=True)
                return
            else:
                ...
                if gen_qr_address is None:
                    await ctx.reply(f"'{EMOJI_RED_NO} Failed to generate QR.", ephemeral=True)
                    return
                if option == "PLAIN":
            ...
            traceback.print_exc(file=sys.stdout)
            await ctx.reply(f"Internal error to get deposit for: {COIN_NAME}. Please report.", ephemeral=True)

...
class Games(commands.Cog):
    ...
    async def game_blackjack(
        ...
        if serverinfo and 'enable_game' in serverinfo and serverinfo['enable_game'] == "NO":
            ...
            return {"error": f"{EMOJI_RED_NO} {ctx.author.mention} Game is not ENABLE yet in this guild. Please request Guild owner to enable by `{prefix}SETTING GAME`"}
        ...
        try:
            ...
            if (datetime.now().astimezone() - account_created).total_seconds() <= config.game.account_age_to_play:
                return {"error": f"{EMOJI_RED_NO} {ctx.author.mention} Your account is very new. Wait a few days before using this."}
            ...
            while not player_over:  # Keep looping until player stands or busts.
                ...
                try:
                    msg = await ctx.reply('{} **BLACKJACK**\n'
                                         ...
                                         '{}\n'
                                         'PLAYER:  {}\n'
                                         '{}```'.format(ctx.author.mention, get_display['dealer_header'], 
                                         get_display['dealer'], get_display['player_header'], get_display['player']), components=[row])
                except Exception as e:
                    await logchanbot(traceback.format_exc())
                    game_over = True
                    if ctx.author.id in GAME_INTERACTIVE_PRGORESS:
                ...
                try:
                    inter = await self.bot.wait_for('button_click', check=check, timeout=60)
                    ...
                    try:
                        ...
                        pass
                    return
                if inter.clicked_button.custom_id == "blackjack_hit":
                    # Hit/doubling down takes another card.
                    newCard = deck.pop()
                    rank, suit = newCard
                    try:
                        await inter.reply('{} **BLACKJACK** You drew a {} of {}'.format(ctx.author.mention, rank, suit))
                    except Exception as e:
                        if ctx.author.id in GAME_INTERACTIVE_PRGORESS:
                            GAME_INTERACTIVE_PRGORESS.remove(ctx.author.id)
                    ...
                    if blackjack_getCardValue(playerHand) >= 21:
                        ...
                        player_over = True
                        break
                elif inter.clicked_button.custom_id == "blackjack_stand":
                    await inter.reply('{} **BLACKJACK** You selected to stand.'.format(ctx.author.mention))
                    player_over = True
                    break
    ...
    async def game_slot(
        ...
        try: 
            ...
            if serverinfo and serverinfo[index_game]:
                if ctx.channel.id != int(serverinfo[index_game]):
                    gameChan = self.bot.get_channel(int(serverinfo[index_game]))
                    if gameChan: return {"error": f"{EMOJI_RED_NO} {ctx.author.mention}, {gameChan.mention} is for game **slot** channel!!!"}
        except Exception as e:
            traceback.print_exc(file=sys.stdout)
            await logchanbot(traceback.format_exc())
        # end of bot channel check
    ...
    async def game_maze(
        ...
        try:
            ...
            while (playerx, playery) != (exitx, exity):
                ...
                if (inter.clicked_button.custom_id == "up" and maze_data[(playerx, playery - 1)] == EMPTY) \
                or (inter.clicked_button.custom_id == "down" and maze_data[(playerx, playery + 1)] == EMPTY) \
                or (inter.clicked_button.custom_id == "left" and maze_data[(playerx - 1, playery)] == EMPTY) \
                or (inter.clicked_button.custom_id == "right" and maze_data[(playerx + 1, playery)] == EMPTY):
                ...
                try:
                    ...
                    await inter.create_response(content=f'{ctx.author.mention} Maze:\n```{maze_edit}```', components=[row], type=dislash.ResponseType.UpdateMessage)
                except Exception as e:
                    await logchanbot(traceback.format_exc())
            if (playerx, playery) == (exitx, exity):
                won = True
    ...
    async def slot(
        ...
        try:
            ...
            if game_slot and "error" in game_slot:
                await ctx.reply(game_slot['error'])
        except Exception as e:
            traceback.print_exc(file=sys.stdout)

...
class Faucet(commands.Cog):
    ...
    async def bot_faucet(
        ...
        for COIN_NAME in [coinItem.upper() for coinItem in FAUCET_COINS]:
            sum_sub = 0
            wallet = await store.sql_get_userwallet(str(self.bot.user.id), COIN_NAME)
            if wallet is None:
                if COIN_NAME in ENABLE_COIN_ERC:
                    coin_family = "ERC-20"
                    w = await create_address_eth()
                    wallet = await store.sql_register_user(str(self.bot.user.id), COIN_NAME, SERVER_BOT, 0, w)
                elif COIN_NAME in ENABLE_COIN_TRC:
                    coin_family = "TRC-20"
                    result = await store.create_address_trx()
                    wallet = await store.sql_register_user(str(self.bot.user.id), COIN_NAME, SERVER_BOT, 0, result)
                else:
                    coin_family = getattr(getattr(config,"daemon"+COIN_NAME),"coin_family","TRTL")
                    wallet = await store.sql_register_user(str(self.bot.user.id), COIN_NAME, SERVER_BOT, 0)
            userdata_balance = await store.sql_user_balance(str(self.bot.user.id), COIN_NAME)
            xfer_in = 0
            if COIN_NAME not in ENABLE_COIN_ERC+ENABLE_COIN_TRC:
                xfer_in = await store.sql_user_balance_get_xfer_in(str(self.bot.user.id), COIN_NAME)
    ...
    async def take_action(
        ...
        if ctx.author.bot == True:
            ...
            return {"error": f"{EMOJI_RED_NO} {ctx.author.mention} Bot is not allowed using this."}

        # Check if tx in progress
        if ctx.author.id in TX_IN_PROCESS: return {"error": f"{EMOJI_ERROR} {ctx.author.mention} You have another tx in progress."}
        remaining = ''
        try:
            tmp_msg = await ctx.send("Loading faucet...")
            remaining = await self.bot_faucet(ctx) or ''
        ...
        if COIN_NAME in ENABLE_COIN+ENABLE_COIN_DOGE+ENABLE_XMR+ENABLE_COIN_NANO+ENABLE_COIN_ERC+ENABLE_COIN_TRC+ENABLE_XCH:
            ...
            user_from = await store.sql_get_userwallet(str(self.bot.user.id), COIN_NAME)
            if user_from is None:
                if COIN_NAME in ENABLE_COIN_ERC:
                    w = await create_address_eth()
                    user_from = await store.sql_register_user(str(self.bot.user.id), COIN_NAME, SERVER_BOT, 0, w)
                elif COIN_NAME in ENABLE_COIN_TRC:
                    result = await store.create_address_trx()
                    user_from = await store.sql_register_user(str(self.bot.user.id), COIN_NAME, SERVER_BOT, 0, result)
                else:
                    user_from = await store.sql_register_user(str(self.bot.user.id), COIN_NAME, SERVER_BOT, 0)
            userdata_balance = await store.sql_user_balance(str(self.bot.user.id), COIN_NAME)
            xfer_in = 0
            if COIN_NAME not in ENABLE_COIN_ERC+ENABLE_COIN_TRC:
                xfer_in = await store.sql_user_balance_get_xfer_in(str(self.bot.user.id), COIN_NAME)
                ...
                try:
                    if not info:
                        if coin_family in ["TRTL", "BCN"]:
                            tip = await store.sql_mv_cn_single(str(self.bot.user.id), str(ctx.author.id), real_amount, 'FAUCET', COIN_NAME)
                        elif coin_family == "XMR":
                            tip = await store.sql_mv_xmr_single(str(self.bot.user.id), str(ctx.author.id), real_amount, COIN_NAME, "FAUCET")
                        elif coin_family == "XCH":
                            tip = await store.sql_mv_xch_single(str(self.bot.user.id), str(ctx.author.id), real_amount, COIN_NAME, "FAUCET")
                        elif coin_family == "NANO":
                            tip = await store.sql_mv_nano_single(str(self.bot.user.id), str(ctx.author.id), real_amount, COIN_NAME, "FAUCET")
                        elif coin_family == "DOGE":
                            tip = await store.sql_mv_doge_single(str(self.bot.user.id), str(ctx.author.id), real_amount, COIN_NAME, "FAUCET")
                        elif coin_family == "ERC-20":
                            token_info = await store.get_token_info(COIN_NAME)
                            tip = await store.sql_mv_erc_single(str(self.bot.user.id), str(ctx.author.id), real_amount, COIN_NAME, "FAUCET", token_info['contract'])
    ...
    async def take(
        ...
        if take_action and "error" in take_action:
            await ctx.reply(take_action['error'])
